export const drinks = [
  // MILK TEAS
  {
    id: "1",
    name: "Classic Milk",
    slug: "classic-milk",
    price: 300,
    image: "/classic.png",
    category: { name: "Classic Milk Tea" },
    description: "The original pearl milk tea that started it all. Rich, creamy, and perfectly balanced.",
    isSpecial: true,
    rating: 4.8
  },
  {
    id: "2",
    name: "Brown Sugar Milk",
    slug: "brown-sugar-milk",
    price: 300,
    image: "/BrownSuga.png",
    category: { name: "Classic Milk Tea" },
    description: "Our signature brown sugar pearls with creamy milk. A fan favorite.",
    isSpecial: true,
    rating: 5.0
  },
  {
    id: "3",
    name: "Thai Milk",
    slug: "thai-milk",
    price: 300,
    image: "/thai.png",
    category: { name: "Classic Milk Tea" },
    description: "A bold, spice-infused black tea with creamy milk and sweetness.",
    rating: 4.7
  },
  {
    id: "4",
    name: "Mango Milk",
    slug: "mango-milk",
    price: 300,
    image: "/mango.png",
    category: { name: "Classic Milk Tea" },
    description: "Sweet mango puree blended with creamy milk tea.",
    rating: 4.6
  },
  {
    id: "5",
    name: "Strawberry Milk",
    slug: "strawberry-milk",
    price: 300,
    image: "/strawberrymilk.png",
    category: { name: "Classic Milk Tea" },
    description: "Fresh strawberry flavors met with our signature creamy base.",
    rating: 4.7
  },

  // FRUIT TEAS
  {
    id: "6",
    name: "Passion Fruit",
    slug: "passion-fruit",
    price: 300,
    image: "/passion.png",
    category: { name: "Fruit Tea" },
    description: "Tangy and refreshing passion fruit mixed with premium green tea.",
    rating: 4.8
  },
  {
    id: "7",
    name: "Strawberry Fruit Tea",
    slug: "strawberry-fruit-tea",
    price: 300,
    image: "/strawberryfruit.png",
    category: { name: "Fruit Tea" },
    description: "Fresh strawberries blended with premium jasmine green tea.",
    rating: 4.7
  },
  {
    id: "8",
    name: "Mango Fruit Tea",
    slug: "mango-fruit-tea",
    price: 300,
    image: "/mangofru.png",
    category: { name: "Fruit Tea" },
    description: "Refreshing green tea infused with real mango pieces.",
    rating: 4.7
  },
  {
    id: "9",
    name: "Jasmine Lemon",
    slug: "jasmine-lemon",
    price: 250,
    image: "/jasmine.png",
    category: { name: "Fruit Tea" },
    description: "Light and floral jasmine tea with a zesty lemon kick.",
    rating: 4.5
  },
  {
    id: "10",
    name: "Wonjo",
    slug: "wonjo",
    price: 250,
    image: "/wonjo.png",
    category: { name: "Fruit Tea" },
    description: "Authentic local Hibiscus tea, brewed to perfection.",
    rating: 4.9
  },
  {
    id: "11",
    name: "Lemon & Mint Ice Tea",
    slug: "lemon-mint-ice-tea",
    price: 250,
    image: "/lemon.png",
    category: { name: "Fruit Tea" },
    description: "A classic cooling blend of lemon and fresh mint.",
    rating: 4.6
  },
  {
    id: "12",
    name: "Smiling Coast (Wonjo & Mango)",
    slug: "smiling-coast",
    price: 300,
    image: "/rose.png",
    category: { name: "Fruit Tea" },
    description: "Our signature tropical blend of Wonjo and Mango.",
    rating: 4.8
  },

  // SPECIALS
  {
    id: "13",
    name: "Iced Latte",
    slug: "iced-latte",
    price: 300,
    image: "/icedlatte.png",
    category: { name: "Specialty Drinks" },
    description: "Smooth espresso shots over ice with milk.",
    rating: 4.7
  },
  {
    id: "14",
    name: "Caramel/Vanilla Iced Latte",
    slug: "caramel-vanilla-latte",
    price: 350,
    image: "/Caramel.png",
    category: { name: "Specialty Drinks" },
    description: "Your choice of sweet caramel or vanilla in our smooth iced latte.",
    rating: 4.8
  },
  {
    id: "15",
    name: "Chai Masala Milk",
    slug: "chai-masala-milk",
    price: 300,
    image: "/chai.png",
    category: { name: "Specialty Drinks" },
    description: "A perfect blend of rich spices, coffee, and our signature milk tea.",
    rating: 4.7
  },
  {
    id: "16",
    name: "Matcha Milk",
    slug: "matcha-milk",
    price: 300,
    image: "/matcha.png",
    category: { name: "Specialty Drinks" },
    description: "Premium Japanese matcha whisked to perfection and served with fresh milk.",
    isSpecial: true,
    rating: 4.9
  },
  {
    id: "17",
    name: "Taro Milk",
    slug: "taro-milk",
    price: 300,
    image: "/tero.png",
    category: { name: "Specialty Drinks" },
    description: "Creamy taro combined with our signature milk tea for a sweet, earthy treat.",
    isSpecial: true,
    rating: 4.7
  },
  {
    id: "18",
    name: "Strawberry Matcha",
    slug: "strawberry-matcha",
    price: 350,
    image: "/strawberrymatcha.png",
    category: { name: "Specialty Drinks" },
    description: "The perfect fusion of earth matcha and sweet strawberries.",
    rating: 4.8
  },
  {
    id: "19",
    name: "Baobab Milk",
    slug: "baobab-milk",
    price: 250,
    image: "/baobabmilk.png",
    category: { name: "Specialty Drinks" },
    description: "A unique, nutritious blend of local baobab and creamy milk.",
    rating: 4.6
  },
  {
    id: "20",
    name: "Pink Paradise (Wonjo & Baobab)",
    slug: "pink-paradise",
    price: 250,
    image: "/pinkparadise.png",
    category: { name: "Specialty Drinks" },
    description: "A colorful fusion of refreshing Wonjo and Baobab.",
    rating: 4.8
  },
  {
    id: "21",
    name: "Ice Cream Boba",
    slug: "ice-cream-boba",
    price: 450,
    image: "/ice.png",
    category: { name: "Specialty Drinks" },
    isSnack: true,
    seasonal: true,
    seasonLabel: "Special Offer 🍦",
    offerLabel: "Buy 1 Get 1 Free",
    description: "Creamy ice cream meet boba pearls in this ultimate dessert drink.",
    rating: 5.0
  },

  // SNACKS
  {
    id: "22",
    name: "Belgian Waffle",
    slug: "belgian-waffle",
    price: 300,
    image: "/zigg.png",
    category: { name: "Snacks" },
    description: "Crispy on the outside, fluffy on the inside. Served with your choice of syrup.",
    rating: 4.9
  },
  {
    id: "23",
    name: "Vanilla Ice Cream",
    slug: "vanilla-ice-cream",
    price: 300,
    image: "/cream.png",
    category: { name: "Snacks" },
    description: "Creamy, classic vanilla bean ice cream.",
    rating: 4.7
  },
  {
    id: "24",
    name: "Chocolate Ice Cream",
    slug: "chocolate-ice-cream",
    price: 300,
    image: "/whitecu.png",
    category: { name: "Snacks" },
    description: "Rich and decadent dark chocolate ice cream.",
    rating: 4.8
  },
  {
    id: "25",
    name: "Strawberry Ice Cream",
    slug: "strawberry-ice-cream",
    price: 300,
    image: "/stick.png",
    category: { name: "Snacks" },
    description: "Sweet strawberry ice cream with real fruit chunks.",
    rating: 4.7
  },

  // COFFEE
  {
    id: "26",
    name: "Cappuccino",
    slug: "cappuccino",
    price: 250,
    image: "/Cappuccino-removebg-preview.png",
    category: { name: "Coffee" },
    description: "Classic Italian coffee with a perfect balance of espresso, steamed milk, and foam.",
    rating: 4.8,
  },
  {
    id: "27",
    name: "Morning Coffee",
    slug: "morning-coffee",
    price: 200,
    image: "/Morning-Coffee.png",
    category: { name: "Coffee" },
    description: "Your daily dose of energy. Freshly brewed premium roast.",
    rating: 4.5,
  },
  {
    id: "28",
    name: "White Cappuccino",
    slug: "white-cappuccino",
    price: 280,
    image: "/white-capachino.png",
    category: { name: "Coffee" },
    description: "Creamy and smooth cappuccino with extra steamed milk for a velvety finish.",
    rating: 4.7,
  },
  {
    id: "29",
    name: "Premium Dark Roast",
    slug: "premium-dark-roast",
    price: 300,
    image: "/Cozy_Coffee_Moment__Rich__Dark_Roast_in_a_Golden_Cup-removebg-preview.png",
    category: { name: "Coffee" },
    description: "Bold and intense dark roast for the true coffee connoisseur.",
    rating: 4.9,
    isSpecial: true,
  },
  {
    id: "30",
    name: "Café Au Lait",
    slug: "cafe-au-lait",
    price: 270,
    image: "/Holiday_Spiced_Café_Au_Lait_-_The_Local_Palate-removebg-preview.png",
    category: { name: "Coffee" },
    description: "Balanced mix of strong coffee and hot milk, perfect for a cozy moment.",
    rating: 4.6,
  },

  // SMOOTHIES
  {
    id: "31",
    name: "Mango Smoothie",
    slug: "mango-smoothie",
    price: 350,
    image: "/Mango_Smoothie.png",
    category: { name: "Smoothies" },
    description: "Thick and creamy smoothie made with sun-ripened tropical mangoes.",
    rating: 4.9,
    isSpecial: true,
  },
  {
    id: "32",
    name: "Tropical Berry Smoothie",
    slug: "tropical-berry-smoothie",
    price: 320,
    image: "/smoothies-removebg-preview.png",
    category: { name: "Smoothies" },
    description: "A refreshing blend of forest berries and tropical fruits.",
    rating: 4.7,
  },
  {
    id: "33",
    name: "Strawberry Berry Smoothie",
    slug: "strawberry-berry-smoothie",
    price: 330,
    image: "/Download_AI_generated_Strawberry_and_berry_smoothie_isolated_on_png_background_for_free-removebg-preview.png",
    category: { name: "Smoothies" },
    description: "Sweet strawberries mixed with a medley of delicious berries.",
    rating: 4.8,
  },
];
